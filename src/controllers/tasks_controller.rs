...
use crate::models::{
    ...
    tasks_model::{CreateTask, Tasks, UpdateTask},
};
...
pub async fn delete_task(pool: &SqlitePool, task_id: i32) -> Result<()> {
...
pub async fn delete_task_by_employee_task_id(
    pool: &SqlitePool,
    employee_id: i32,
    project_id: i32,
) -> Result<()> {
    sqlx::query("DELETE FROM tasks WHERE employee_id = $1 AND project_id = $2")
        .bind(employee_id)
        .bind(project_id)
        .execute(pool)
        .await?;
